...
@Repository
public interface XpressRepository extends CrudRepository<PrestamoEntity, String> {
    //------------------------------------------------------------------------------------------------------------------
    /*METHODS*/
    //------------------------------------------------------------------------------------------------------------------
    @Query("SELECT pr.agente AS agencia, " +
            "COUNT(pr) AS clientes, " +
            "SUM(CASE WHEN pr.diaDePago = 'MIERCOLES' THEN CASE WHEN pa.cierraCon < pr.tarifa THEN pa.cierraCon " +
            "       ELSE pr.tarifa END ELSE 0 END) AS debitoMiercoles, " +
            "SUM(CASE WHEN pr.diaDePago = 'JUEVES' THEN CASE WHEN pa.cierraCon < pr.tarifa THEN pa.cierraCon " +
            "       ELSE pr.tarifa END ELSE 0 END) AS debitoJueves, " +
            "SUM(CASE WHEN pr.diaDePago = 'VIERNES' THEN CASE WHEN pa.cierraCon < pr.tarifa THEN pa.cierraCon " +
            "       ELSE pr.tarifa END ELSE 0 END) AS debitoViernes, " +
    ...
    @Query("SELECT pr.agente AS agencia, " +
            ...
            "SUM(CASE WHEN pa.monto < pr.tarifa AND pa.cierraCon > 0 AND pa.monto > 0 THEN 1 ELSE 0 END) " +
            "       AS pagosReducidos, " +
            "SUM(CASE WHEN pr.diaDePago = 'MIERCOLES' THEN CASE WHEN pa.abreCon < pr.tarifa THEN pa.abreCon " +
            "       ELSE pr.tarifa END ELSE 0 END) AS debitoMiercoles, " +
            "SUM(CASE WHEN pr.diaDePago = 'JUEVES' THEN CASE WHEN pa.abreCon < pr.tarifa THEN pa.abreCon " +
            "       ELSE pr.tarifa END ELSE 0 END) AS debitoJueves, " +
            "SUM(CASE WHEN pr.diaDePago = 'VIERNES' THEN CASE WHEN pa.abreCon < pr.tarifa THEN pa.abreCon " +
            "       ELSE pr.tarifa END ELSE 0 END) AS debitoViernes, " +
            "SUM(CASE WHEN pa.abreCon < pr.tarifa THEN pa.abreCon ELSE pr.tarifa END) AS debitoTotal, " +
            "ROUND(((SUM(pa.monto) - SUM(CASE WHEN pa.monto > pr.tarifa THEN pa.monto - pr.tarifa ELSE 0 END)) " +
            "       / SUM(CASE WHEN pa.abreCon < pr.tarifa THEN pa.abreCon ELSE pr.tarifa END) * 100), 2) " +
            "       AS rendimiento, " +
            "SUM(CASE WHEN pr.descuento > 0 THEN pr.descuento ELSE 0 END) AS totalDeDescuento, " +
            "SUM(pa.monto) - SUM(CASE WHEN pa.monto > pr.tarifa THEN pa.monto - pr.tarifa ELSE 0 END) " +
            "       AS totalCobranzaPura, " +
            ...
            "WHEN pa.abreCon < pr.tarifa AND pa.monto < pa.abreCon THEN pa.abreCon - pa.monto ELSE 0 END) " +
            "       AS montoDeDebitoFaltante, " +
