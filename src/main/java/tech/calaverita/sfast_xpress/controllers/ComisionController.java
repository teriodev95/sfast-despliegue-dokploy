...
public class ComisionController {
    ...
    public ComisionController(CalendarioService calendarioService, AgenciaService agenciaService,
            PrestamoViewService prestamoViewService, PagoDynamicService pagoDynamicService, VentaService ventaService,
    ...
    public @ResponseBody ResponseEntity<ComisionModel> postCreateOne(@RequestBody ComisionModel comisionModel) {
        ...
        HttpStatus responseHttpStatus = responseComisionModel != null ? HttpStatus.CREATED : HttpStatus.CONFLICT;
    ...
    public @ResponseBody ResponseEntity<ComisionModel> getCurrentWeek(@PathVariable String agencia) {
        ...
        CompletableFuture<Double> debitoTotalCF = this.prestamoViewService
                .findDebitoTotalByAgenciaSaldoAlIniciarSemanaGreaterThanAndNotAnioAndSemana(agencia, 0D, anio, semana);
        ...
        CompletableFuture<Double> cobranzaPuraHastaJuevesCF = this.pagoDynamicService
                .findCobranzaPuraByAgenciaAnioSemanaAndFechaPagoLessThanEqual(agencia, anio, semana, fechaJueves);
        ...
        CompletableFuture<ArrayList<VentaModel>> ventaModelsCF = this.ventaService.findByAgenciaAnioAndSemanaAsync(
                agencia,
                anio, semana);
        ...
        comisionModel.setNivel(
                BalanceAgenciaUtil.getNivelAgente(clientesPagoCompleto, cobranzaPuraHastaJueves, agenteUsuarioModel));
        ...
        comisionModel
                .setComisionCobranza(MyUtil.getDouble(
                        (cobranzaTotal + primerosPagos) / 100 * comisionModel.getPorcentajeComisionCobranza()));
