...
public class ApartadoCasetas extends GeneradoraTablasIngresosEgresos {
    ...
    public PdfPTable creaTablaCasetas(TablaFlujoEfectivoGerente data) throws DocumentException {
        PdfPTable table = new PdfPTable(2);
        table.setWidthPercentage(100);

        table.addCell(lineaVacia(table));
        table.addCell(generarEncabezadoIngresosConceptoEgresos("CASETAS", table.getNumberOfColumns())).setBorder(0);
        ...
        for (Egreso egreso : egresosCasetas) {
            ...
            cell.setBorderColor(rosa);
            tablaEgresos.addCell(cell);
            cell = estiloCeldaCentrado(new Phrase(" ", regularFont));
            cell.setBorderColor(rosa);
            tablaEgresos.addCell(cell);

        }
        rellenoFilasNulas(egresosCasetas, tablaEgresos, 5, rosa);
        ...
        if (data.getCasetas().getSubTotal() == 0) {
            tablaEgresos.addCell(new PdfPCell(estiloCeldaMonto(new Phrase("-", regularFont)))).setBorderColor(rosa);
        } else {
            tablaEgresos
                    .addCell(new PdfPCell(
                            estiloCeldaMonto(new Phrase(String.valueOf(data.getCasetas().getSubTotal()), regularFont))))
                    .setBorderColor(rosa);
        }
