...
public class ApartadoAsignacionesSeguridad extends GeneradoraTablasIngresosEgresos {
    ...
    public PdfPTable creaTablasAsignacionesSeguridad(TablaFlujoEfectivoGerente data) throws DocumentException {
        PdfPTable table = new PdfPTable(2);
        table.setWidthPercentage(100);

        table.addCell(lineaVacia(table));
        table.addCell(generarEncabezadoIngresosConceptoEgresos("ASIGNACIONES SEGURIDAD", table.getNumberOfColumns()))
                .setBorder(0);
        ;

        // TABLA INGRESOS
        List<Ingreso> ingresosAsignacionesSeguridad = data.getAsignacionesSeguridad().getIngresos();
        String[] headers = { "FECHA: ", "RECIBI DE: ", "MONTO $" };
        PdfPTable tablaIngresos = generarTablaYEncabezados(headers, azulClaro);
        for (Ingreso ingreso : ingresosAsignacionesSeguridad) {
        ...
        agregarRellenoYSubtotal(ingresosAsignacionesSeguridad, tablaIngresos, 3,
                data.getAsignacionesAdministracion().getSubTotalIngresos(), azulClaro);

        // TABLA EGRESOS
        List<Egreso> egresosAsignacionesSeguridad = data.getAsignacionesSeguridad().getEgresos();
        String[] headersE = { "FECHA: ", "ASIGNE A: ", "MONTO $" };
        PdfPTable tablaEgresos = generarTablaYEncabezados(headersE, rosa);

        for (Egreso egreso : egresosAsignacionesSeguridad) {
        ...
        agregarRellenoYSubtotal(egresosAsignacionesSeguridad, tablaEgresos, 3,
