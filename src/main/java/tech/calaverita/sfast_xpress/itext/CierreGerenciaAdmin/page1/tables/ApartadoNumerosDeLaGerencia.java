...
import tech.calaverita.sfast_xpress.itext.CierreGerenciaAdmin.page1.tables.classes.TablaNumerosGerencia.ClienteMetrica;
import tech.calaverita.sfast_xpress.itext.CierreGerenciaAdmin.page1.tables.classes.TablaNumerosGerencia.Metrica;
import tech.calaverita.sfast_xpress.itext.CierreGerenciaAdmin.page1.tables.classes.TablaNumerosGerencia.MetricasClientes;
import tech.calaverita.sfast_xpress.itext.CierreGerenciaAdmin.page1.tables.classes.TablaNumerosGerencia.MetricasFinancieras;
import tech.calaverita.sfast_xpress.itext.CierreGerenciaAdmin.page1.tables.classes.TablaNumerosGerencia.ObjetivosVentas;
...
public class ApartadoNumerosDeLaGerencia {
    ...
    public PdfPTable creaTablaNumerosDeLaGerencia(TablaNumerosGerencia data) throws DocumentException {
        ...
        PdfPTable tablaMetricasFinancieras = new PdfPTable(4); // 4 columnas: Concepto, Semana Anterior, Actual,
                                                               // Diferencia
        tablaMetricasFinancieras.addCell(new PdfPCell(new Phrase(" ", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                .setBorder(0);
        tablaMetricasFinancieras
                .addCell(new PdfPCell(new Phrase("Semana Ant.", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                .setBorder(0);
        tablaMetricasFinancieras
                .addCell(new PdfPCell(new Phrase("Actual", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                .setBorder(0);
        tablaMetricasFinancieras
                .addCell(new PdfPCell(new Phrase("Diferencia", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                .setBorder(0);
        ...
        PdfPTable tablaMetricasClientes = new PdfPTable(5); // 4 columnas: Concepto, Anterior, Actual, Diferencia
        tablaMetricasClientes.setWidths(new float[] { 1, 2, 1, 1, 1 });
        tablaMetricasClientes.addCell(bloqueVacioRelleno(2));
        tablaMetricasClientes.addCell(new PdfPCell(new Phrase("Ant.", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                .setBorder(0);
        tablaMetricasClientes.addCell(new PdfPCell(new Phrase("Actual", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                .setBorder(0);
        tablaMetricasClientes.addCell(new PdfPCell(new Phrase("Dif.", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                .setBorder(0);
        ...
        tablaMetricasClientes.addCell(bloqueVacioRelleno(2));
        PdfPCell conceptoDesc = new PdfPCell(new Phrase("Desq. x Liq.", FontFactory.getFont(FontFactory.HELVETICA, 7)));
        ...
        tablaMetricasClientes.addCell(new PdfPCell(new Phrase(String.valueOf(clientes.getDescuentosPorLiquidacion()),
                FontFactory.getFont(FontFactory.HELVETICA, 7)))).setHorizontalAlignment(Element.ALIGN_CENTER);

        // TABLA OBJETIVOS VENTAS
        PdfPTable tablaObjetivosVentas = new PdfPTable(3); // 5 columnas: Objetivo, Actual, Ventas Nuevas, Renovaciones,
                                                           // Total
    ...
    private static void agregaFilaMetricasClientes(PdfPTable tabla, String concepto, ClienteMetrica metrica) {
        ...
        if (concepto == "# P.Reducidos") {
            if (metrica.getAnterior() == 0) {
                tabla.addCell(new PdfPCell(new Phrase(" ", FontFactory.getFont(FontFactory.HELVETICA, 7))))
                        .setBorder(0);
            ...
            tabla.addCell(new PdfPCell(
                    new Phrase(String.valueOf(metrica.getAnterior()), FontFactory.getFont(FontFactory.HELVETICA, 7))));
            tabla.addCell(new PdfPCell(
                    new Phrase(String.valueOf(metrica.getActual()), FontFactory.getFont(FontFactory.HELVETICA, 7))));
            tabla.addCell(new PdfPCell(new Phrase(String.valueOf(metrica.getDiferencia()),
                    FontFactory.getFont(FontFactory.HELVETICA, 7))));
        }
    }
    ...
    private static void agregaFilaMetricasFinancieras(PdfPTable tabla, String concepto, Metrica metrica) {
        ...
        if (concepto == "Eficiencia") {
            ...
            tabla.addCell(new PdfPCell(new Phrase(
                    metrica.getSemanaAnterior() != 0 ? "$ " + formatoMonto.format(metrica.getSemanaAnterior()) : "$ - ",
                    FontFactory.getFont(FontFactory.HELVETICA, 6)))).setHorizontalAlignment(Element.ALIGN_RIGHT);
            tabla.addCell(new PdfPCell(
                    new Phrase(metrica.getActual() != 0 ? "$ " + formatoMonto.format(metrica.getActual()) : "$ - ",
                            FontFactory.getFont(FontFactory.HELVETICA, 6, azulClaro))))
                    .setHorizontalAlignment(Element.ALIGN_RIGHT);
            tabla.addCell(new PdfPCell(new Phrase(
                    metrica.getDiferencia() != 0 ? "$ " + formatoMonto.format(metrica.getDiferencia()) : "$ - ",
