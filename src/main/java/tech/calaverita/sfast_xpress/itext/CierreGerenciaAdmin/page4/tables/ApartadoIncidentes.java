...
public class ApartadoIncidentes extends GeneradoraTablasIngresosEgresos {
    ...
    public PdfPTable creaTablaIncidentes(TablaFlujoEfectivoGerente data) throws DocumentException {
        PdfPTable table = new PdfPTable(2);
        table.setWidthPercentage(100);

        table.addCell(lineaVacia(table));
        table.addCell(generarEncabezadoIngresosConceptoEgresos("INCIDENTES (Accidentes, robos, gastos medicos)",
                table.getNumberOfColumns())).setBorder(0);
        ;

        // TABLA INGRESOS
        List<Ingreso> ingresosIncidentes = data.getIncidentes().getIngresos();
        String[] headers = { "FECHA: ", "COMENTARIO", "MONTO $" };
        PdfPTable tablaIngresos = generarTablaYEncabezados(headers, azulClaro);

        for (Ingreso ingreso : ingresosIncidentes) {
        ...
        agregarRellenoYSubtotal(ingresosIncidentes, tablaIngresos, 2, data.getIncidentes().getSubTotalIngresos(),
                azulClaro);

        // TABLA EGRESOS
        List<Egreso> egresosIncidentes = data.getIncidentes().getEgresos();
        String[] headersE = { "FECHA: ", "COMENTARIO", "MONTO $" };
        PdfPTable tablaEgresos = generarTablaYEncabezados(headersE, rosa);
        for (Egreso egreso : egresosIncidentes) {
        ...
        agregarRellenoYSubtotal(egresosIncidentes, tablaEgresos, 2, data.getIncidentes().getSubTotalEgresos(), rosa);
