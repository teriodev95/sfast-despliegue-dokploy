...
public class ApartadoAsignacionesAdministracion extends GeneradoraTablasIngresosEgresos {
    ...
    public PdfPTable creaTablasAsignacionesAdimnistracion(TablaFlujoEfectivoGerente data) throws DocumentException {
        PdfPTable table = new PdfPTable(2);
        table.setWidthPercentage(100);

        table.addCell(lineaVacia(table));
        table.addCell(generarEncabezadoIngresosConceptoEgresos("ASIGNACIONES ADMINISTRACIÓN (Caja chica)",
                table.getNumberOfColumns())).setBorder(0);
        ;

        // TABLA INGRESOS
        List<Ingreso> ingresosAsignacionesAdministracion = data.getAsignacionesAdministracion().getIngresos();
        String[] headers = { "FECHA: ", "RECIBI DE: ", "MONTO $" };
        PdfPTable tablaIngresos = generarTablaYEncabezados(headers, azulClaro);
        for (Ingreso ingreso : ingresosAsignacionesAdministracion) {
        ...
        agregarRellenoYSubtotal(ingresosAsignacionesAdministracion, tablaIngresos, 3,
                data.getAsignacionesAdministracion().getSubTotalIngresos(), azulClaro);

        // TABLA EGRESOS
        List<Egreso> egresosAsignacionesAdministracion = data.getAsignacionesAdministracion().getEgresos();
        String[] headersE = { "FECHA: ", "ASIGNE A: ", "MONTO $" };
        PdfPTable tablaEgresos = generarTablaYEncabezados(headersE, rosa);

        for (Egreso egreso : egresosAsignacionesAdministracion) {
        ...
        agregarRellenoYSubtotal(egresosAsignacionesAdministracion, tablaEgresos, 3,
                data.getAsignacionesAdministracion().getSubTotalEgresos(), rosa);
