...
public class NumerosGerenciaMetricasClientesDto {
        ...
        public NumerosGerenciaMetricasClientesDto(List<CobranzaAgencia> cobranzaAgenciaSemanaAnterior,
                        List<CobranzaAgencia> cobranzaAgenciaSemanaActual) {
                this();

                int clientesSemanaAnterior = cobranzaAgenciaSemanaAnterior.stream()
                                .mapToInt(CobranzaAgencia::getClientes)
                                .sum();
                int noPagosSemanaAnterior = cobranzaAgenciaSemanaAnterior.stream().mapToInt(CobranzaAgencia::getNoPagos)
                                .sum();
                int pagosReducidosSemanaAnterior = cobranzaAgenciaSemanaAnterior.stream()
                                .mapToInt(CobranzaAgencia::getPagosReducidos).sum();
                int clientesLiquidadosSemanaAnterior = cobranzaAgenciaSemanaAnterior.stream()
